...
description: "Creates an industry BENCHMARKING REPORT comparing the user's metrics against industry medians and top-quartile performers. Pulls current benchmarks from reputable sources (SaaS Benchmarks, OpenView, ChartMogul, a16z, YC) via web research, builds a scorecard, gap analysis, and target recommendations. Use when evaluating where the business stands versus peers."
...
Benchmarks provide context, not answers. A metric that sits below benchmark is a signal to investigate — the gap may reflect a genuine problem, a strategic tradeoff, or a difference in stage, geography, or segment. Treat every gap as a question first and a problem second.
...
Use `WebSearch` and `WebFetch` to pull the most RECENT benchmarks (last 12-18 months). Always cite source + year for every number. Prefer these sources:
...
**If sources conflict**, report the range (e.g., "median churn 2-4% depending on source") rather than a single number.
