...
Ad spend is an investment equation: every dollar in should produce a measurable return, and the math should work before the first dollar is spent. This skill owns the math only — for multi-channel allocation and roadmap, hand off to `media-buy-plan`.

## How to run the math

Two options, pick one:

1. **Automated (preferred)** — run `scripts/calc.py` with the inputs from Phase 1. It returns the full metric table, viability flags, and three scenarios in one pass. Usage:

   ```bash
   python scripts/calc.py --revenue 10000 --aov 197 --margin 0.85 --cvr 0.03 --cpc 2.00
   ```

2. **Manual** — fill the tables in Phase 2-3 with the formulas shown. Use when you don't have a Python runtime or want to explain the math step-by-step to the user.
