...
Every dollar of ad spend should have a job — testing a new channel, scaling a proven one, or retargeting warm audiences. No budget should be allocated without a clear expected outcome and a way to measure it.

## Scope and delegation

- **This skill** owns: multi-channel allocation, testing roadmap, monthly/quarterly pacing, decision rules for scale/kill, seasonal adjustments.
- **`ad-spend-calculator`** owns: unit economics (AOV, margin, max CPA, target ROAS). Run it first to get the numbers that feed Phase 2's budget allocation.
- **`facebook-ad-campaign` / `google-ads-campaign`** own: the tactical build of campaigns once the channel is funded.
- **`ad-performance-report`** owns: the weekly/monthly reporting cadence that informs the decision rules here.
...
Do not re-derive unit economics here. Run `ad-spend-calculator` first and paste the resulting max CPA, target CPA, target ROAS, and LTV-based max CPA into the brief. Those numbers become the performance bar that every channel in Phase 3 must clear.
