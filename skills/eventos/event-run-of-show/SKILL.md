...
description: "Creates detailed run-of-show documents with minute-by-minute timing, team roles, AV cues, transition scripts, and contingency plans for live or virtual events. Use when a user is producing a conference, webinar, workshop, launch event, or any live production that needs an operational document to keep speakers, AV, hosts, and volunteers synchronized during execution."
allowed-tools:
  - Read
  - Write
metadata:
...
A run-of-show is the single document every team member references during the event. If something is not written into it — a cue, a transition, a contingency, an owner — it will not happen reliably under the pressure of live production. Treat the run-of-show as the source of truth: when in doubt, add it to the document rather than relying on memory or verbal agreement.
