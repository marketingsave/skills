...
Fix the biggest leak first. A 10% improvement at the worst drop-off point usually beats a 50% improvement at a stage that already converts well, because absolute volume recovered is what moves revenue — not percentage uplift in isolation. Always rank stages by absolute customers lost, not by conversion rate alone.
...
Deliver the analysis as:

```
# Funnel Analysis — [Funnel Name] — [Date Range]

## Funnel Snapshot
| Stage | Volume | Conv % | Drop-off % | vs. Benchmark |
|-------|--------|--------|-----------|---------------|
| ...   | ...    | ...    | ...       | ...           |

## Biggest Leak
Stage: [name] — losing X customers/mo. Absolute opportunity: $Y.
...
## Action Plan
| Fix | Expected Uplift | Effort | Owner | Measurement |
|-----|----------------|--------|-------|-------------|
```
