...
description: "Generates a 30-day OMNICHANNEL editorial content calendar as a live Notion database (with posts mapped to pillars, platforms, and dates) plus starter Canva graphic templates per pillar. Covers blog, newsletter, podcast, YouTube long-form, AND social. Use when the user wants an operational, tool-backed calendar (Notion + Canva artifacts). For social-only planning without Notion/Canva, or for a 30-day batch-recordable short-form video plan, use `social-media/social-content-calendar` instead."
allowed-tools:
  - Read
  - Write
...
- Adapting a single existing piece of content into multiple platform formats — use `content-repurpose` instead. This skill plans NEW content; `content-repurpose` adapts EXISTING content.
- **Social-only planning when the user does NOT want Notion/Canva artifacts** — use `social-media/social-content-calendar` (pure markdown, weekly/monthly, supports `full-mix` and `video-heavy` modes).
- **30-day batch-recordable short-form video plans** (TikTok/Reels/Shorts with hooks, trending adaptation, batch-recording sessions) — use `social-media/social-content-calendar` in `video-heavy` mode.
- **Social strategy / pillar definition from scratch** — use `social-media/social-platform-strategy` first, then return here for the omnichannel operational calendar.
...
Default: 4 pillars — Education, Authority, Connection, Promotion. Add Community only if the user's strategy relies on audience engagement (polls, questions, UGC).
...
Default: 5 posts/week (Standard) — covers weekdays and leaves weekends free for batch creation, which is the cadence most solo operators can actually sustain.
...
Every calendar starts with content pillars. Generating post ideas without pillars first produces random content that doesn't compound: individual posts may perform, but the account never builds a recognizable point of view that drives followers and conversions over time.
...
Gate: do not proceed to Step 2 until you have at minimum the business/niche, platforms, and either user-provided or default-assigned content pillars.
...
See `references/examples.md` for two end-to-end walkthroughs:
- Fitness coach (Instagram + LinkedIn, 5 posts/week, with launch event)
- SaaS founder (X/Twitter + LinkedIn, 3 posts/week)
...
- **Generating post ideas without pillars first** — random ideas can't be batched (no shared frameworks to reuse) and don't compound into a recognizable brand voice.
- **More than 25% promotional content** — audiences unfollow when every post is a pitch; promotion works because it's rare, not because it's frequent.
- **Same post type on the same platform 3+ days in a row** — variety is what the algorithm (and the viewer) rewards.
- **Creating Canva templates before the user approves the calendar** — templates depend on confirmed pillars; premature generation wastes time if pillars change.
- **Leaving the Canva Link property empty without explaining the workflow** — the user won't know the template-to-post mapping and the system falls apart on use.
- **Assuming posting days** — ask the user or default to weekdays (Mon-Fri); weekend posting has very different audience behavior.
- **Front-loading promotional posts in week 1** — a cold audience needs education and connection first; leading with promotion trains them to ignore the account.
