...
- Validate every finding against a live tool (GSC, PageSpeed Insights, Rich Results Test, URL Inspection) before recommending a fix — never rely on inference alone.
- Prioritize fixes by blast radius: issues that block crawling/indexing come before Core Web Vitals, which come before on-page polish.
- Deliver findings as a prioritized checklist with concrete verification steps, not a list of generic best practices.
...
- **Massive crawl errors after migration:** Implement a redirect map immediately (old URL → new URL, 301s, no chains). Use the `seo-migration-plan` skill for the full recovery playbook and re-submit the sitemap in GSC.
