...
After the sequence is sent, prospects reply in one of four ways: positive, objection, rejection, or silence. Full response scripts (with example copy for each scenario) are in `references/response-scripts.md`. Load that file when a reply comes in or when delivering the sequence package to the user.

Quick rule for silence: **never send a fourth email in the same sequence.** Mark as "sequence completed — no reply" and revisit only if a new trigger event appears in 60-90 days.
